...
function ENT:PlaySound(sound,speed)
	...
	self.SoundObj[sound]:ChangeVolume(GetConVar("zcm_cl_sfx_volume"):GetFloat() or 1, 0)
...
function ENT:Draw()
	...
	if zcm.f.InDistance(LocalPlayer():GetPos(), self:GetPos(), 400) then

		local nAng = self:GetAngles()
		nAng:RotateAroundAxis(self:GetForward(), 90)
		nAng:RotateAroundAxis(self:GetUp(), 180)
		...
		cam.Start3D2D(attach.Pos, nAng, 0.05)
			...
			if table.Count(zcm.config.CrackerMachine.Upgrades.Ranks) > 0 then
				if table.HasValue(zcm.config.CrackerMachine.Upgrades.Ranks, LocalPlayer():GetUserGroup()) then
					self:DrawUpgrade()
				end
			else
				self:DrawUpgrade()
...
function ENT:Think()
	...
	-- ClientModel
	if zcm.f.InDistance(LocalPlayer():GetPos(), self:GetPos(), 600) then
		...
		if self.ClientProps then
			if self.LastBlackPowder ~= self:GetBlackPowder() then
				...
				if self.LastBlackPowder > 0 then
					ParticleEffect("zcm_blackpowder", self:LocalToWorld(Vector(-79, -26, 92)), self:GetAngles(), NULL)
				end
			end
		...
		elseif self.ProductionStage01 == 3 then
			...
			timer.Simple(0.5 / speed,function()
				if IsValid(_RollCutter) then
					ParticleEffectAttach("zcm_cutting_dust", PATTACH_POINT_FOLLOW, _RollCutter, 1)
				end
			end)
		...
		elseif self.ProductionStage02 == 2 then
			...
			timer.Simple(2 / speed,function()
				if IsValid(_RollBinder) then
					ParticleEffectAttach("zcm_warpping", PATTACH_POINT_FOLLOW, _RollBinder, 1)
				end
			end)
		elseif self.ProductionStage02 == 3 then
		    ...
		    timer.Simple(6.5 / speed,function()
		        if IsValid(self) then
		     		ParticleEffect("zcm_blackpowder_burst", self:LocalToWorld(Vector(-79, -26, 28)), self:GetAngles(), NULL)
		        end
		    end)
		end
	end
...
function ENT:UpdatePaper_Visual()
	...
	if self.LastPaper <= 0 then
		...
		local newScale = ((1 / zcm.config.CrackerMachine.Paper_Cap) * self.LastPaper) + 0.2
...
function ENT:UpdateBlackPowder_Visual()
	if self.LastBlackPowder <= 0 then
		...
		local newheight = (14 / zcm.config.CrackerMachine.BlackPowder_Cap) * self.LastBlackPowder
...
function ENT:SpawnClientModel_Paper()
	local attach = self:GetPaperRoller():GetAttachment(1)
	local ent = ents.CreateClientProp("models/zerochain/props_crackermaker/zcm_paper_cyl.mdl")
...
function ENT:SpawnClientModel_BlackPowder()
	local ent = ents.CreateClientProp("models/zerochain/props_crackermaker/zcm_powder.mdl")
