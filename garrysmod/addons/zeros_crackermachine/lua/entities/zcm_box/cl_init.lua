...
function ENT:DrawButtons()
	...
	cam.Start3D2D(self:LocalToWorld(Vector(0, -15, 23)), ang, 0.05)
		...
		if zcm.config.SellBox.DirectSell then
			...
			local mText = zcm.config.Currency .. (earning * 8)
			draw.SimpleText(mText, "zcm_font06", 0, -550, zcm.default_colors["black01"], TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
			draw.SimpleText(mText, "zcm_font05", 0, -550, zcm.default_colors["green04"], TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP)
		end

		local atext = ""
		if zcm.config.SellBox.DirectSell then
			atext = zcm.language.General["Sell"]
		else
			atext = zcm.language.General["Collect"]
		end
...
function ENT:Draw()
	...
	if zcm.f.InDistance(LocalPlayer():GetPos(), self:GetPos(), 400) and self.IsOpen == false then
		if table.Count(zcm.config.SellBox.Jobs) > 0 then
			if table.HasValue(zcm.config.SellBox.Jobs, team.GetName(LocalPlayer():Team())) then
				self:DrawButtons()
			end
		else
			self:DrawButtons()
...
function ENT:SpawnClientModel_Firework(pos)
	local ent = ents.CreateClientProp("models/zerochain/props_crackermaker/zcm_fireworkpack.mdl")
