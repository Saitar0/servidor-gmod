...
function GAS.Logging:FormatEntity(ent)
	...
	elseif (ent:IsVehicle()) then
		...
		if (IsValid(driver)) then
			GAS.Logging.CurrentLog[1][rIndex] = {GAS.Logging.FORMAT_VEHICLE, ent:GetClass(), ent:GetModel(), driver:AccountID(), GAS.Logging:PersistentPlayerData(driver)}
		else
...
function GAS.Logging:FormatVehicle(ent, no_driver)
	local rIndex = #GAS.Logging.CurrentLog[1] + 1

	if (no_driver == true) then
		GAS.Logging.CurrentLog[1][rIndex] = {GAS.Logging.FORMAT_VEHICLE, ent:GetClass(), ent:GetModel()}
	else
		GAS.Logging.CurrentLog[1][rIndex] = {GAS.Logging.FORMAT_VEHICLE, ent:GetClass(), ent:GetModel(), GAS.Logging:PersistentPlayerData(ent:GetDriver())}
	end
...
GAS.Logging.LiveLogsPlayers = {}
GAS.Logging.LiveLogsPlayers_Count = 0
GAS.Logging.LiveLogsIn10Seconds = 0

GAS:netReceive("logging:LiveLogs", function(ply)
	if (not OpenPermissions:HasPermission(ply, "gmodadminsuite/logging") or not OpenPermissions:HasPermission(ply, "gmodadminsuite/logging/see_live_logs")) then return end
	GAS.Logging.LiveLogsPlayers[ply] = true
	GAS.Logging.LiveLogsPlayers_Count = GAS.Logging.LiveLogsPlayers_Count + 1
end)
GAS:hook("PlayerDisconnect", "logging:RemoveFromLiveLogs", function(ply)
	if (GAS.Logging.LiveLogsPlayers[ply]) then
		GAS.Logging.LiveLogsPlayers[ply] = nil
		GAS.Logging.LiveLogsPlayers_Count = GAS.Logging.LiveLogsPlayers_Count - 1
	end
end)
...
GAS:hook("Tick", "logging:ExtraProcessingQueue", function()
	...
	if (logtbl) then
		...
		if (GAS.Logging.Config.LiveLogsEnabled and not GAS.Logging.Modules:IsLiveLogsDisabled(module)) then
			...
			if (GAS.Logging.LiveLogsPlayers and GAS.Logging.LiveLogsPlayers_Count > 0) then
				if (GAS.Logging.LiveLogsIn10Seconds > GAS.Logging.Config.LiveLogsIn10Seconds) then
					if (GAS.Logging.Config.NotifyLiveLogsAntispam) then
						for ply in pairs(GAS.Logging.LiveLogsPlayers) do
							if (not IsValid(ply)) then continue end
							if (not OpenPermissions:IsOperator(ply)) then
								if (not OpenPermissions:HasPermission(ply, "gmodadminsuite/logging", false) or not OpenPermissions:HasPermission(ply, "gmodadminsuite/logging/see_live_logs", false)) then continue end
		...
		local allow_discord_send = discord_webhooks and (GAS_Logging_Discord_DailyQuota_Today == nil or os.date("%d-%m-%Y") ~= GAS_Logging_Discord_DailyQuota_Today) and (GAS_Logging_Discord_RateLimitedUntil == nil or os.time() >= GAS_Logging_Discord_RateLimitedUntil)
		if (allow_discord_send) then
			...
			local function send_discord_request()
				local function execute(webhook_i)
					...
					print(discord_webhooks[webhook_i])
					PrintTable(discord_webhooks[webhook_i] or {})

					HTTP({
						method = "POST",
						url = (discord_webhooks[webhook_i][2]:gsub("discordapp%.com/api/webhooks", "billys-logs-discord.party")),
						...
						success = function(code, body, headers)
							if (code == 403 and body:sub(1,11) == "DAILY_QUOTA") then
								GAS:print("[bLogs] Your daily webhook quota of " .. body:sub(13) .. " requests has been exceeded until tomorrow", GAS_PRINT_COLOR_BAD, GAS_PRINT_TYPE_WARN)
								GAS_Logging_Discord_DailyQuota_Today = os.date("%d-%m-%Y")
...
local function store_player_columns(log_id, i, account_id, persistent_data)
	...
	if (persistent_data ~= nil) then -- persistent player data
		...
		if (persistent_data[4] ~= nil) then -- health
			data[9] = math.Clamp(persistent_data[4], 0, 65535)
		end
		if (persistent_data[5] ~= nil) then -- armor
...
function GAS.Logging.DeepStorage:ProcessLog(logtbl)
	...
	for i,replacement in ipairs(logtbl[1]) do
		...
		elseif (replacement[1] == GAS.Logging.FORMAT_DAMAGE) then
			...
			GAS.Database:Query("INSERT INTO " .. GAS.Database:ServerTable("gas_logging_deepstorage_logdata") .. " (`log_id`, `data_index`, `dmg_amount`, `dmg_type`) VALUES(" .. logtbl[7] .. "," .. i .. "," .. math.Clamp(tonumber(replacement[2]), 0, 65535) .. "," .. math.Clamp(tonumber(replacement[3]), 0, 4294967295) .. ")")

		elseif (replacement[1] == GAS.Logging.FORMAT_CURRENCY) then

			GAS.Database:Query("INSERT INTO " .. GAS.Database:ServerTable("gas_logging_deepstorage_logdata") .. " (`log_id`, `data_index`, `currency`) VALUES(" .. logtbl[7] .. "," .. i .. "," .. tonumber(replacement[2]) .. ")")
