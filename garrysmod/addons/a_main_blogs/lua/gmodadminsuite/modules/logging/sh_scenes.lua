...
if (SERVER) then
	...
	function GAS.Logging.Scenes:SetupScene(scene_id, beginning, data, origin_callback)
		...
		local victim, victim_wep = GAS.Logging.Scenes:CreatePlayer(scene_obj[1])
		local instigator, instigator_wep = GAS.Logging.Scenes:CreatePlayer(scene_obj[2])

		GAS.Logging.Scenes.SceneEnts[k] = {victim, victim_wep, instigator, instigator_wep}
		...
		local instigator_eyes = instigator:LookupAttachment("eyes")
		...
		local instigator_head, victim_head = instigator:LookupBone("ValveBiped.Bip01_Head1"), victim:LookupBone("ValveBiped.Bip01_Head1")
		...
		GAS.Logging.Scenes.HUDDraw[k] = function()
			...
			if (not instigator_head or instigator_head == 0) then
				...
				instigator_origin = instigator:GetBonePosition(instigator_head) + Vector(0,0,10)
			end
