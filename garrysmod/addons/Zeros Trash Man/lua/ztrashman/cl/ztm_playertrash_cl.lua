...
function ztm.f.PostDrawOpaqueRenderables_PlayerTrash()
	...
	if near_players and table.Count(near_players) > 0 then
		...
		for k, v in pairs(near_players) do
			if IsValid(v) and v:Alive() and v ~= LocalPlayer() then
				...
				draw.DrawText(v:GetNWInt("ztm_trash", 0) .. ztm.config.UoW, "ztm_playertrash_font02", pos.x, pos.y + (-10 * hMod), ztm.default_colors["black02"], TEXT_ALIGN_CENTER)
				draw.DrawText(v:GetNWInt("ztm_trash", 0) .. ztm.config.UoW, "ztm_playertrash_font01", pos.x, pos.y + (-10 * hMod), ztm.default_colors["white01"], TEXT_ALIGN_CENTER)
			end
		end
	end
end

hook.Add("HUDPaint", "HUDPaint_playertrash", function()
	if ztm.config.PlayerTrash.Enabled then
		ztm.f.PostDrawOpaqueRenderables_PlayerTrash()
	end
end)
