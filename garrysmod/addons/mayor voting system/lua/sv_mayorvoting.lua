...
function VOTING.SetupDarkRPTeam()
	...
	if VOTING.OnlyEnterUsingNPC then 
		VOTING.Team.CustomCheckFailMsg = "Você deve falar com o NPC para entrar na eleição!"
		return false
	else
		VOTING.Team.CustomCheckFailMsg = "Por favor, confirme sua candidatura à prefeitura!"
		SendUserMessage("VOTING_Confirm", ply)
		return false
	end
	end
	TEAM.CustomCheckFailMsg = "S'il vous plaît confirmer votre candidature à la mairie!"
...
function VOTING.CastVote(ply,cmd,args)
	...
	if not VOTING.AllowCandidatesToVote and ply.HasEnteredVoting then 
		VOTING.SendDarkRPNotice(ply, 1, 4, "Desculpe, os candidatos não podem votar na eleição.") 
		return 
	end
	...
	else VOTING.SendDarkRPNotice(ply, 1, 4, "Este candidato não pode ser votado.") end
end
...
function VOTING.EnterMayorVote(ply, cmd, args)
	...
	if VOTING.NextElectionAllowed and not (CurTime() >= VOTING.NextElectionAllowed) then
		local timemsg = string.format("Você deve esperar mais %i minutos para entrar!", math.Round((VOTING.NextElectionAllowed - CurTime()) / 60 ))
	...
	if not VOTING.AllowNewElectionWithMayor then
		for k,v in pairs(player.GetAll()) do
			if (v:Team() == VOTING.TeamID) then
				VOTING.SendDarkRPNotice(ply, 1, 4, "Já há um prefeito eleito!")
				return false
			end
		end
	end
	...
	if VOTING.InProgress then
		VOTING.SendDarkRPNotice(ply, 1, 4, "Já há uma eleição em andamento!")
	return false end
	
	if ply.HasEnteredVoting then
		VOTING.SendDarkRPNotice(ply, 1, 4, "Você já entrou nesta eleição.")
	return false end
	...
	if (#VOTING.CurrentCandidates) >= VOTING.MaximumCandidates then
		VOTING.SendDarkRPNotice(ply, 1, 4, "Já existem " .. tostring(VOTING.MaximumCandidates) .. " candidatos, desculpe." )
	return false end
	...
	if VOTING.CandidateCost and VOTING.CandidateCost > 0 then
		...
		VOTING.SendDarkRPNotice(ply, 1, 4, "Você não pode pagar as taxas de entrada, desculpe.")
		return false end
	...
	VOTING.SendDarkRPNotice(ply, 2, 4, "Você foi inscrito nas eleições do prefeito. Boa sorte!")
	...
	if (#VOTING.CurrentCandidates >= VOTING.MinimumCandidates) then
		VOTING.AboutToBegin = true
			VOTING.ChatNotice("A próxima eleição do prefeito começará em " .. tostring(VOTING.AboutToBeginTime) .. " segundos!")
		...
		function()
			...
			VOTING.ChatNotice("A votação está prestes a começar. Boa sorte!")
...
if not VOTING.InProgress then return end
	...
	if (winningvotes == 0) then
		VOTING.ChatNotice("Nenhum voto foi registrado na eleição.")
	...
	elseif IsValid(candidate) then
		...
		net.Send(player.GetAll())
		VOTING.ChatNotice(string.format("Parabéns ao vencedor, %s!", candidate:Nick()))
		--Update player team
		...
		VOTING.ChatNotice("O candidato vencedor à prefeitura saiu do servidor!")
		net.Start("Voting_EndVote")
		net.Send(player.GetAll())
	end
...
if IsValid(vic) and VOTING.TeamID == vic:Team() then
	if VOTING.DemoteMayorOnDeath then
		...
		VOTING.ChatNotice("O prefeito foi morto e um novo prefeito deve ser eleito!")
...
function VOTING.PlaceNPC(ply, cmd, args)
	...
	VOTING.SendDarkRPNotice(ply, 0, 4, "O NPC foi colocado na sua posição exata!") 
...
hook.Add("canPocket","VOTING_CantPocketNPC", function(owner, ent)
	if IsValid(ent) and ent:GetClass() == "npc_mayorvoting" then return false, "Você não pode guardar o NPC!" end
end)