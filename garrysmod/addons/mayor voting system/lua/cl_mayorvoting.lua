...
function VOTING.OpenVoteScreen( settings )
	...
	if !VotingMainWindow then
		...
		for k,v in pairs(settings.Candidates) do
			...
			VotingPanel:SetText("Desconectado")
			end
...
function VOTING.CloseVoteScreen()
	if VotingMainWindow then
		...
		VOTING.VoteTickerMessage = "Um voto foi colocado na eleição."
	 ...
	 if winningplayer and not (winningplayer == NULL) and VOTING.VoteManager then
			...
			for k,v in pairs(VOTING.VoteManager) do
				if not (v:GetPlayer() == winningplayer) then
					...
					VOTING.VoteTickerMessage = string.format("Parabéns ao vencedor, %s!", winningplayer:Nick())
	 ...
	 for k,v in pairs(VOTING.VoteManager) do
		if (v:GetPlayer() == candidate) then
			...
			if VOTING.Settings.ShowVoteTickerUpdates and IsValid(player) and IsValid(candidate) then
				VOTING.VoteTickerAlpha = 0
				VOTING.VoteTickerMessage = string.format("%s votou por %s", player:Nick(), candidate:Nick())
				VOTING.VoteTickerColor = v:GetColor()
			end
		end
	 end
...
function VOTING.ConfirmCandidacy()
	...
	Derma_Query("Você quer participar das próximas eleições? " .. LocalPlayer():Nick() .. " ?", "Secretária do prefeito",
			"Sim " .. (CUR or GAMEMODE.Config.currency or "$") .. tostring(VOTING.CandidateCost), function() LocalPlayer():ConCommand("mayor_vote_enter") ConfirmMenuVisible = false end,
			"Cancelar", function() ConfirmMenuVisible = false end
			)
end
