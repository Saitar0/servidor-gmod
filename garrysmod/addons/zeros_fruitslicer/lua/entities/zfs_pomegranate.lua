...
function ENT:Finish_VFX_SFX()
    self:CreateEffect_Table(nil, "zfs_sfx_pomegranate_finish", self, self:GetAngles(), self:GetPos())

    return self.BaseClass.Finish_VFX_SFX(self)
end

function ENT:Interact_VFX_SFX()
    self:CreateEffect_Table("zfs_melon", "zfs_sfx_pomegranate", self, self:GetAngles(), self:GetPos())
